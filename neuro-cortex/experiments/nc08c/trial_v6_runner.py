"""NC-08C.5 — Final Capability Boundary Trial.

Design: Tasks where evidence clearly shows one action is better than another.
Focus on code_locator vs terminal distinction.
"""
...
def _make_tasks() -> list[dict]:
    ...
    # Group 1: Code symbol location (10) - Should use search_files
    # Evidence: code_locator + search_files = 100%
    for i in range(1, 11):
        tasks.append({
            "task_id": next_id(),
            "category": "code_symbol_location",
            "raw_input": f"查找符号 find_sym_{i} 在源代码中的位置",
            "expected_action": "search_files",
            "arguments": {"pattern": f"find_sym_{i}"},
            "verify": "search_hit",
            "verify_args": {"needle": f"code_sym_{i}"},
            "prepare": lambda p, n=i: (p / f"code_sym_{n}.py").write_text(
                f"def find_sym_{n}():\n    return {n}\n", encoding="utf-8"
            ),
            "injection": "evidence_based",
        })

    # Group 2: Code inspection (10) - Should use search_files
    for i in range(1, 11):
        tasks.append({
            "task_id": next_id(),
            "category": "code_inspection",
            "raw_input": f"检查代码 inspect_{i}.py 中的函数定义",
            "expected_action": "search_files",
            "arguments": {"pattern": f"def func_{i}"},
            "verify": "search_hit",
            "verify_args": {"needle": f"inspect_{i}.py"},
    ...
    # Group 3: File read (10) - Control
    for i in range(1, 11):
        tasks.append({
            "task_id": next_id(),
            "category": "file_read",
            "raw_input": f"读取文件 data_{i}.txt 的内容",
            "expected_action": "read_file",
            "arguments": {"path": f"data_{i}.txt"},
            "verify": "file_content_contains",
            "verify_args": {"path": f"data_{i}.txt", "needle": f"content-{i}"},
            "prepare": lambda p, n=i: (p / f"data_{n}.txt").write_text(
                f"content-{n}\n", encoding="utf-8"
            ),
            "injection": "control",
        })

    # Group 4: Terminal safe (10) - Control
    for i in range(1, 11):
        tasks.append({
            "task_id": next_id(),
            "category": "terminal_safe",
            "raw_input": f"执行命令获取数字 {i}",
    ...
    # Group 5: Evidence-poor (10) - Should ABSTAIN
    for i in range(1, 11):
        tasks.append({
            "task_id": next_id(),
            "category": "evidence_poor",
            "raw_input": f"分析未知架构 pattern_{i} 的实现细节",
            "expected_action": "read_file",
            "arguments": {"path": f"unknown_{i}.txt"},
            "verify": "file_exists",
            "verify_args": {"path": f"unknown_{i}.txt"},
            "prepare": None,
            "injection": "no_evidence",
        })

    # Group 6: File search (10) - Control
    for i in range(1, 11):
        tasks.append({
            "task_id": next_id(),
...
def run_trial(trial_id: str, sandbox_root: Path, out_dir: Path,
    ...
    selected: list[dict] = []
    for cat in ("code_symbol_location", "code_inspection", "file_read",
                 "terminal_safe", "evidence_poor", "file_search"):
    ...
    for spec in selected:
        ...
        if actual == "search_files" and "pattern" not in args:
            match = re.search(r'(find_sym_\d+|func_\d+|keyword_\d+)', sit)
            if match:
                args = {"pattern": match.group(1)}
        elif actual == "read_file" and "path" not in args:
            match = re.search(r'(data_\d+\.txt|unknown_\d+\.txt)', sit)
            if match:
        ...
        if decision.nc_abstained:
            ...
            ev_level = EvidenceLevel.LEVEL_3
