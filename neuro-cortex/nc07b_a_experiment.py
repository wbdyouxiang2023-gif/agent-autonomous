#!/usr/bin/env python3
"""NC-07B-A: Controlled Policy Outcome Trial"""
...
class OutcomeTrial:
    ...
    def _simulate(self, task, action, group):
        h = int(hashlib.md5(f"{task}{action}{group}".encode()).hexdigest()[:8], 16)
        if action == "tool_call":
    ...
    def process_task(self, task, intent, group):
        ...
        cands = [ActionLearningCandidate(a, a) for a in ["code_edit", "code_review", "tool_call", "respond"]]
        ranked = self._engine.rank_actions(sit, cands)
        policy_result = self._policy.choose_action(sit, cands, ranked)
        nc_action = policy_result.get("selected_action")
        actual_action = original_action if group == "CONTROL" else nc_action
        outcome = self._simulate(task, actual_action, group)
...
def main():
    ...
    if e_comp_rate < c_comp_rate - 0.3:
        print("\n⚠️  WARNING: NC respond avoids execution (lower task completion)")
    
    report = f"""# NC-07B-A: Controlled Policy Outcome Trial

**Date**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
**Verdict**: `NO_VALIDATED_POLICY_SUPERIORITY` (pilot)

## Summary
- CONTROL: 10 tasks (tool_call)
- EXPLORATION: 10 tasks (respond)

## Results
| Metric | CONTROL | EXPLORATION |
|--------|---------|-------------|
| Success Rate | {c_rate:.1%} | {e_rate:.1%} |
| Task Completion | {c_comp_rate:.1%} | {e_comp_rate:.1%} |

## Key Finding
{'⚠️ NC respond has LOWER task completion - may be avoiding execution' if e_comp_rate < c_comp_rate else 'Both policies similar'}

## Verdict
`NO_VALIDATED_POLICY_SUPERIORITY`
- Sample size insufficient (10 per group)
- Critical: Need to measure BOTH success AND completion
"""
